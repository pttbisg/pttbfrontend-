...
import {
  ...
  generateInventoryDataExport,
} from "../../tables/react-tables/TableData";
...
class Inventory extends React.Component {
  ...
  render() {
    const { agData, data, columnDefs, defaultColDef, isLoading, dataExport } =
      this.state;
    let renderTableData = dataExport.map((col) => {
      return (
        <tr
          key={`${col.masterSKU}&${
            col.balanceStockLeft * Math.random()
          }&${new Date().getTime()}`}
        >
          <td>{col.masterSKU}</td>
            ...
            <CardBody className="py-0">
              <div className="ag-theme-material w-100 my-2">
                ...
                <>
                  ...
                  <div className="ag-grid-tbl">
                    {agData && agData.length > 0 && (
                      <AgGridReact
                        gridOptions={{}}
