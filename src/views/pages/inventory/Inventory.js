...
class Inventory extends React.Component {
  ...
  renderSwitch(key, expanded, param, rowData) {
    switch (key) {
      case 'inbounds':
        return (
          <React.Fragment>
            <div onClick={() => this.onExpanded(rowData, key, expanded)} className="title text-bold-600 text-left"
                 style={{minWidth: 300}}>
                            <span style={{cursor: 'pointer'}}>{expanded ? <ChevronUp/> : <ChevronDown/>} Inbounds
                                <span
                                  className="mx-1 badge badge-primary badge-pill">{param && param.length > 0 ? param.length : 0}</span>
                            </span>
            </div>
            {expanded && <ReactTable
              data={param}
  ...
  _renderInboundData = (item) => {
    return <>
      `<tr colSpan="5">
        <td className="tg-0lax" colSpan="5">{item[0].toUpperCase()}</td>
      </tr>
      <tr>
        <td>Date</td>
        <td>Location</td>
      ...
      {
        item[1].child.map((child, index) => {
          return <tr key={index}>
            <td className="tg-0lax">{child.date || ''}</td>
            <td className="tg-0lax">{child.current_location || ''}</td>
            <td className="tg-0lax">{child.qty || ''}</td>
          </tr>
        })
      }
      <tr>
        <td>Total</td>
        <td>{item[1].child.reduce((total, item)=> {
          total += item.qty;
          return total;
        },0)}</td>
      </tr>
  ...
  _renderSalesData = (item) => {
    return <>
      ...
      </tr>
      {
        item[1].child.map((child, index) => {
          return <tr key={index}>
            <td className="tg-0lax">{child.storename || ''}</td>
            <td className="tg-0lax">{child.qty || ''}</td>
          </tr>
        })
      }
  ...
  _renderOtherUseData = (item) => {
    return <>
      ...
      </tr>
      {
        item[1].child.map((child, index) => {
          return <tr key={index}>
            <td className="tg-0lax">{child.reason || ''}</td>
            <td className="tg-0lax">{child.qty || ''}</td>
          </tr>
        })
      }
  ...
  render() {
    ...
    let renderTableData = data?.map(col => {
        return (
          <tbody key={`${col.masterSKU}&${col.balanceStockLeft*Math.random()}&${new Date().getTime()}`}>
          <tr>
            <td colSpan={5}></td>
          </tr>
          <tr>
            <td className="title" colSpan="5">SKU : {col.masterSKU}</td>
          </tr>
          {Object.entries(col.inventory_details)
            .map((item) => {
            if (item[0] === 'inbounds') {
              return this._renderInboundData(item);
            }
            if (item[0] === 'sales') {
              return this._renderSalesData(item);
            }

            return this._renderOtherUseData(item);
          })}
          <tr>
            <td>BALANCE STOCK LEFT:</td>
            <td>{col.balance_stock_left}</td>
          </tr>
          </tbody>
        )
      }
              ...
              <div className='ag-theme-material w-100 my-2'>
                ...
                <>
                  {data && data.length > 0 ?
                      ...
                      columns={[
                        {
                          ...
                          Cell: props => {
                            ...
                            return (
                              <div>
                                <div
                                  className='text-uppercase text-bold-600 text-left navbar-light pt-1 pb-1 px-1 mb-1'>{rowData.masterSKU || ''}</div>
                                <div className="">
                                ...
                                <div
                                  className='d-flex flex-column flex-md-row justify-content-end navbar-light pt-1 pb-1 px-1'>
                    ...
                    /> : isLoading ? <div className="text-center">Getting Data...</div> :
